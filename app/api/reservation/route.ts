...
export async function POST(req: Request) {
  ...
  if (!projectId || !token) {
    ...
    return NextResponse.json(
      { ok: false, error: 'Serverkonfiguration saknas. Maila oss på info@ifmba.se.' },
      { status: 500 },
    )
  }
  ...
  if (resendKey) {
    const adminTo = process.env.RESERVATION_NOTIFY_TO || process.env.LEAD_NOTIFY_TO || 'info@ifmba.se'
    ...
    const custHtml = `
      <div style="font-family:Inter,system-ui,sans-serif;max-width:560px">
        ...
        <hr style="border:0;border-top:1px solid #eee;margin:24px 0">
        <p style="color:#777;font-size:12px">Frågor? Svara på det här mejlet eller skriv till <a href="mailto:info@ifmba.se">info@ifmba.se</a></p>
